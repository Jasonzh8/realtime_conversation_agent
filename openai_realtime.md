...
Typically, you'll want to accept the incoming call as shown below.

|URL|https://api.openai.com/v1/realtime/calls/$CALL_ID/accept|
|Payload|{ "type: realtime", "model": "gpt-realtime", ...} (similar to session.update)|
|Headers|Authorization: Bearer YOUR_API_KEYSubstitute YOUR_API_KEY with a standard API key|

Reject the call
---------------

Alternatively, you can also reject the incoming call:

|URL|https://api.openai.com/v1/realtime/calls/$CALL_ID/reject|
|Payload|{ "sip_code": code } This SIP response code (e.g., 486 to indicate "busy") will be sent back to the caller in the INVITE response.|
|Headers|Authorization: Bearer YOUR_API_KEYSubstitute YOUR_API_KEY with a standard API key|

Monitor call events
-------------------

Once connected, you can establish a WebSocket and send and receive events to the realtime session, just like a typical Realtime API WebSocket. Note that whe connecting to an existing call using the `call_id` parameter, the `model` argument is not used (as it has already been configured via the `accept` endpoint).
...
It's also possible to redirect an accepted call to another number, via a SIP REFER request. During the call, make a POST to the `refer` endpoint:

|URL|https://api.openai.com/v1/realtime/calls/$CALL_ID/refer|
|Payload|{ "target_uri": uri } This target_uri value will be the value used in the Refer-To header. You can use a tel: URI, for example: tel:+14152909007|
|Headers|Authorization: Bearer YOUR_API_KEYSubstitute YOUR_API_KEY with a standard API key|

Hang up the call
----------------

Lastly, you can end an accepted call from the application side. The call will terminate immediately when this endpoint is invoked.

|URL|https://api.openai.com/v1/realtime/calls/$CALL_ID/hangup|
|Payload|None|
|Headers|Authorization: Bearer YOUR_API_KEYSubstitute YOUR_API_KEY with a standard API key|
